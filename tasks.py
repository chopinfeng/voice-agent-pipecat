...
# 唯一一类真正**动手改东西**的任务。其余四类都只看不写——那是有意为之的（见
# tools.py 顶部的说明：语音很容易把「上一级目录」听成「商议及目录」，写操作的代价
# 不可挽回）。这类存在的前提是：写操作**必须收在一个能审计的口子里**，不能是随便
# 一条 shell 命令都能触发的重定向——所以 Bash 在这里也**不能**直接写文件，落盘只能
# 走 ``Edit``/``Write``（Claude 侧）或 ``write_file``（内建侧），一次调用就是一次
# 完整、可读的「这个文件现在长这样」，不用去猜一串命令拼出来的最终结果。
# 也只有派单时**显式**选了 dev，模型才拿得到这些工具——听岔了、随口一句话触发的
# 默认路径（general/codebase/compute/research）碰不到写权限，这是这类任务唯一的
# 安全网，没有另外加自动提交或二次确认。改动前手动 commit 一下，万一改错了能直接
# 用 git revert，这个提醒写在这里，而不是代码里替你自动做。
DEV = Kind(
    name="dev",
    desc="要动手改的：写代码、改已有文件、跑测试——除了看和搜，还要写和跑测试验证",
    prompt=(
        "你要动手实现一个改动：写代码、改已有文件、跑测试确认没错。项目在 {root}，"
        "只能改这个目录下的文件，不要碰这个目录以外的任何东西，也不要碰 .git 内部"
        "或任何 .env 文件——那些不是你该动的。\n"
        "**先用 Read/Glob/Grep 看清楚现状再动手**，改之前把要改的文件真正读一遍，"
        "别凭文件名或函数名猜内容。\n"
        "落盘只能用 Edit（改已有文件的一部分）或 Write（整份重写一个文件），"
        "Bash 里的输出重定向（比如 `echo x > file`）**一律会被拒绝**——那不是给你"
        "写文件用的，是用来跑命令看结果的。\n"
        "改完之后**跑一遍测试或者跑一遍确认没错**，跑不通就说清楚哪里错了，"
        "不要在没验证过的情况下说「改好了」。\n"
        "说结果时**只说改了什么、验证过没有**，不要把中间读了哪些文件、想了什么"
        "过程复述一遍——那些是说给自己听的，不是说给用户听的。\n" + SPEAK
    ),
    tools=["Read", "Glob", "Grep", "Bash", "Edit", "Write"],
    # 改代码要来回读、改、跑测试、看报错再改，比单纯查代码的轮数预算高一截。
    max_turns=40,
    builtin_tools=["bash", "write_file"],
)

KINDS = {k.name: k for k in (COMPUTE, CODEBASE, RESEARCH, GENERAL, DEV)}
